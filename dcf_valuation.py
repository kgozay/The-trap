...
# Custom CSS to match the "Light Financial Terminal" aesthetic
st.markdown("""
    <style>
...
st.sidebar.caption("Financial Terminal v2.0")
...
with st.sidebar.expander("Macro & Rates", expanded=True):
    rf = st.slider("Risk-free Rate (SARB) %", 7.0, 14.0, 10.5, 0.1)
    erp = st.slider("Equity Risk Premium %", 4.0, 10.0, 6.5, 0.1)
    beta = st.slider("Beta", 0.5, 2.0, def_beta, 0.05)
    inflation = st.slider("Inflation (CPI) %", 3.0, 8.0, 4.5, 0.1)
    zar_impact = st.slider("ZAR Margin Impact (pts)", -5.0, 5.0, 0.0, 0.5)

# --- Operating Inputs ---
with st.sidebar.expander("Operating Drivers", expanded=True):
    revenue_base = st.number_input("Base Revenue (Rm)", value=25000, step=100)
    rev_growth = st.slider("Revenue Growth %", -5.0, 25.0, def_rev, 0.5)
    ebit_margin = st.slider("EBIT Margin %", 0.0, 40.0, def_marg, 0.5)
    depr_pct = st.slider("Depreciation % Rev", 1.0, 10.0, 3.0, 0.1)
    tax_rate = st.slider("Tax Rate %", 20.0, 35.0, 27.0, 1.0)
...
with st.sidebar.expander("Reinvestment Strategy", expanded=False):
    ...
    if input_mode:
        ...
        target_roic = st.slider("Target ROIC %", 5.0, 40.0, 15.0, 0.5)
        # Placeholder for calc variables
        capex_pct, wc_pct = 0, 0
    else:
        capex_pct = st.slider("Capex % Revenue", 1.0, 15.0, 4.0, 0.1)
        wc_pct = st.slider("WC % Revenue", 0.0, 25.0, 10.0, 0.5)
        target_roic = 0

    g = st.slider("Terminal Growth (g) %", 0.0, 8.0, def_g, 0.1)
...
with st.sidebar.expander("Capital Structure", expanded=False):
    ...
    cost_of_debt = st.slider("Cost of Debt %", 5.0, 18.0, 11.0, 0.1)
    debt_weight = st.slider("Debt Weight %", 0.0, 90.0, 25.0, 5.0)

# --- M&A ---
with st.sidebar.expander("M&A / Synergies", expanded=False):
    enable_ma = st.checkbox("Enable Synergies")
    if enable_ma:
        rev_syn = st.slider("Revenue Synergies %", 0.0, 20.0, 0.0, 1.0)
        cost_syn = st.slider("Margin Uplift %", 0.0, 10.0, 0.0, 0.5)
        integ_costs = st.number_input("Integration Costs (Rm)", 0)
    else:
...
with col1:
    st.markdown(f"**Implied Share Price**\\nR {res['share_price']:.2f}")

with col2:
    st.markdown(f"**Enterprise Value**\\nR {res['ev']/1000:.1f}bn")

with col3:
    spread = res['roic_y1'] - res['wacc']
    st.markdown(f"**ROIC Spread**\\n{spread*100:.1f}%")

with col4:
    st.markdown(f"**Implied EV/EBITDA**\\n{res['implied_ev_ebitda']:.1f}x")
